...
class AFK(commands.Cog):
    ...
    async def afk(
        self, ctx: discord.ApplicationContext, reason="No Reason Given!"
    ):
        ...
        embed.set_author(name=ctx.author, icon_url=ctx.bot.user.avatar.url)
    ...
    async def on_message(self, message):
        ...
        if message.author.id in afks.keys():
            ...
            await message.channel.send(
                f"Welcome back {message.author.name}, I removed you AFK"
            )
        ...
        for id, reason in afks.items():
            ...
            if (
                ...
                await message.reply(
                    f"{member.name} is AFK Because ***{reason}***"
                )
